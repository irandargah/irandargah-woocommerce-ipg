...
class WC_Gateway_IranDargah extends WC_Payment_Gateway
{
    ...
    {
        $this->form_fields['merchantId']['description'] .= '<br /><strong>' . __('Sandbox is currently in use.', 'irandargah-woocommerce-ipg') . '</strong>';
    }
    ...
    public function admin_options()
    {
        if (in_array(get_woocommerce_currency(), $this->available_currencies)) {
            ...
            ?>
			<h3><?php _e('IranDargah', 'irandargah-woocommerce-ipg');?></h3>
			<div class="inline error"><p><strong><?php _e('Gateway Disabled', 'irandargah-woocommerce-ipg');?></strong> <?php echo sprintf(__('Choose Iranian Rial or Iranian Toman as your store currency in %1$sGeneral Settings%2$s to enable the IranDargah Gateway.', 'irandargah-woocommerce-ipg'), '<a href="' . esc_url(admin_url('admin.php?page=wc-settings&tab=general')) . '">', '</a>'); ?></p></div>
			<?php
        }
    }
    ...
    public function handle_payment_complete($data, $order)
    {
        ...
        $order->add_order_note(__('payment has been completed', 'irandargah-woocommerce-ipg'));
    ...
    public function handle_payment_failed($data, $order)
    {
        ...
        $this->irandargah_display_failed_message(self::get_order_prop($order, 'id'));
        wp_redirect($woocommerce->cart->get_checkout_url());
     ...
     * @param $order_id
     */
    private function irandargah_display_failed_message($order_id)
    {
        $refid = get_post_meta($order_id, 'irandargah_refid', true);
    ...
    {
        $notice = '';
        $notice .= __('There is no order number referenced.', 'irandargah-woocommerce-ipg');
        $notice .= '<br/>';
        $notice .= __('Please try again or contact the site administrator in case of a problem.', 'irandargah-woocommerce-ipg');
        wc_add_notice($notice, 'error');
    }
    ...
    public function admin_notices()
    {
        ...
        . __('IranDargah requires a merchant id to work.', 'irandargah-woocommerce-ipg')
            . '</p></div>';
    }
}
